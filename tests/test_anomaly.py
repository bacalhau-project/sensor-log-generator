...
class TestAnomalyGenerator:
        ...
        return {
            "anomalies": {
                ...
                "types": {
                    ...
                    AnomalyType.TREND.value: {
                        "enabled": True,
                        "weight": 1.0,
                        "duration_seconds": 300,
                    },
                    AnomalyType.NOISE.value: {
                        "enabled": True,
                        "weight": 1.0,
    ...
    def test_apply_spike_anomaly(self):
        ...
        params_changed = any(
            modified.get(key) != reading.get(key) for key in ["temperature", "humidity", "voltage"]
        )
    ...
    def test_apply_missing_data_anomaly(self):
        ...
        modified, is_anomaly, anomaly_type = generator.apply_anomaly(
            reading, AnomalyType.MISSING_DATA
        )
    ...
    def test_model_affects_anomaly_weights(self):
        ...
        config = self.get_valid_config()
        config["anomalies"]["types"][AnomalyType.MISSING_DATA.value] = {
            "enabled": True,
            "weight": 1.0,
        }
