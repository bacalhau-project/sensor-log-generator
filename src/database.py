...
class DatabaseConnectionManager:
    ...
    def checkpoint(self, mode="PASSIVE"):
        """
        Perform a checkpoint operation to ensure data is written to disk.

        Args:
            mode: Checkpoint mode for WAL (PASSIVE, FULL, RESTART, TRUNCATE)
                  Ignored in DELETE mode

        Returns:
            Tuple of (wal_pages, checkpointed_pages) for WAL mode, None for DELETE mode
        """
        with self._checkpoint_lock:
            try:
                ...
                if journal_mode == "WAL":
                    # In WAL mode, perform actual checkpoint
                    cursor = conn.cursor()
                    ...
                    if self.debug_mode:
                        self.logger.debug(f"WAL checkpoint ({mode}): {result}")
                    ...
                    if self.debug_mode:
                        self.logger.debug(f"Data committed in {journal_mode} journal mode")
    ...
    def periodic_checkpoint(self):
        """Perform periodic checkpoint/commit to ensure data is on disk."""
    ...
    def close_thread_connection(self):
        ...
        if hasattr(self._local, "conn") and self._local.conn:
            try:
                ...
                # Check journal mode to determine if we need checkpoint or commit
                try:
                    cursor = self._local.conn.cursor()
                    cursor.execute("PRAGMA journal_mode;")
                    journal_mode = cursor.fetchone()[0].upper()
                    ...
                    if journal_mode == "WAL":
                        ...
                        self.logger.info(
                            "Executing final WAL checkpoint before connection close..."
                        )
                        cursor = self._local.conn.cursor()
                        cursor.execute("PRAGMA wal_checkpoint(TRUNCATE);")
                        cursor.close()
                    else:
                        # Force a final commit before closing (DELETE mode)
                        self.logger.info("Executing final commit before connection close...")
                        self._local.conn.commit()
                except Exception as e:
                    self.logger.warning(f"Final checkpoint/commit failed: {e}")
...
class SensorDatabase:
        ...
        if self.db_path != ":memory:" and Path.exists(self.db_path):
            if not preserve_existing_db:
                ...
                if not self._check_database_integrity(self.db_path):
                    self.logger.error(f"Existing database at '{self.db_path}' is corrupted!")
        ...
        try:
            ...
            if self.db_path != ":memory:" and Path.exists(self.db_path):
                ...
                if db_size_mb > self.max_database_size_mb:
                    ...
                    self.logger.warning(
                        f"Database size ({db_size_mb:.2f}MB) approaching limit ({self.max_database_size_mb}MB)"
                    )
            ...
            if batch_memory_mb > self.max_memory_usage_mb:
                self.logger.error(
                    f"Batch buffer memory ({batch_memory_mb:.2f}MB) exceeds limit ({self.max_memory_usage_mb}MB)"
                )
                return False
            ...
            if len(self.batch_buffer) > self.max_batch_size:
                self.logger.warning(
                    f"Batch buffer size ({len(self.batch_buffer)}) exceeds max ({self.max_batch_size}), forcing commit"
                )
                self.commit_batch()
        ...
        with self.circuit_breaker_lock:
            if self.circuit_breaker_state == "closed":
                ...
                if (
                    ...
                    self.logger.info(
                        "Circuit breaker entering half-open state, attempting recovery"
                    )
                    return True
                else:
                    return False
    ...
    def _attempt_database_recovery(self, db_path: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    f"Recovered {len(recovered_data)} readings from corrupted database"
                )
            except Exception as e:
            ...
            if recovered_data:
                ...
                for row in recovered_data:
                    try:
                        ...
                        new_cursor.execute(
                            f"INSERT INTO sensor_readings VALUES (NULL, {placeholders})", row[1:]
                        )
                    except Exception as e:
                ...
                try:
                    with self._commit_lock:
                        ...
                        if self.batch_buffer and batch_age >= self.batch_timeout:
                            self.logger.debug(
                                f"Background thread committing {len(self.batch_buffer)} readings"
                            )
                            self.commit_batch()
        ...
        try:
            with self.conn_manager.get_cursor() as cursor:
                ...
                for field_name, field in SensorReadingSchema.model_fields.items():
                    ...
                    sqlite_type = (
                        "TEXT"  # Default to TEXT if type is complex or not directly mappable
                    )
            ...
            if "disk I/O error" in error_msg:
                ...
                if self.failure_retry_count >= len(self.failure_buffer_retry_intervals):
                    ...
                    if len(self.failure_buffer) == 0:
                        ...
                        self.logger.debug(
                            f"Disk I/O error detected, buffering reading (retry in {self.failure_buffer_retry_intervals[0]}s)"
                        )
                    else:
                        # Subsequent failures during retry window
                        self.logger.debug(
                            f"Disk I/O error, buffering reading (retry #{self.failure_retry_count + 1})"
                        )
        ...
        with self._commit_lock:
            ...
            if len(self.batch_buffer) >= self.batch_size or batch_age >= self.batch_timeout:
                if self.debug_mode:
                    ...
                    self.logger.debug(
                        f"Triggering batch commit (reason: {reason}, size: {len(self.batch_buffer)}, age: {batch_age:.1f}s)"
                    )
    ...
    def get_database_stats(self) -> dict:
        ...
        try:
            ...
            stats["performance_metrics"] = {
                ...
                "avg_insert_time_ms": (self.total_insert_time / self.insert_count * 1000)
                if self.insert_count > 0
                else 0,
    ...
    def _add_to_failure_buffer(self, reading_data: SensorReadingSchema):
        ...
        with self.failure_buffer_lock:
            ...
            self.logger.debug(
                f"Added reading to failure buffer (current size: {len(self.failure_buffer)})"
            )
    ...
    def _retry_failed_writes(self):
        ...
        with self.failure_buffer_lock:
            ...
            if self.failure_retry_count >= len(self.failure_buffer_retry_intervals):
                ...
                self.logger.info(
                    f"Attempting to retry {len(self.failure_buffer)} failed writes (retry #{self.failure_retry_count + 1}, interval: {current_retry_interval:.0f}s)"
                )
            else:
            ...
            for reading in self.failure_buffer[:100]:  # Process up to 100 at a time
                try:
                    ...
                    if "disk I/O error" in str(e):
                        ...
                        next_interval = self.failure_buffer_retry_intervals[
                            min(
                                self.failure_retry_count,
                                len(self.failure_buffer_retry_intervals) - 1,
                            )
                        ]
                        ...
                        if self.failure_retry_count >= len(self.failure_buffer_retry_intervals):
                            ...
                            self.logger.debug(
                                f"Disk I/O error on retry #{self.failure_retry_count}, next retry in {next_interval:.0f}s"
                            )
                        break
                    else:
            ...
            if successful_writes:
                ...
                if self.failure_retry_count >= len(self.failure_buffer_retry_intervals):
                    self.logger.info(
                        f"Successfully wrote {len(successful_writes)} readings from failure buffer"
                    )
                    if len(self.failure_buffer) > 0:
                    ...
                    if len(self.failure_buffer) > 0:
                        self.logger.debug(
                            f"Remaining in failure buffer: {len(self.failure_buffer)}"
                        )
